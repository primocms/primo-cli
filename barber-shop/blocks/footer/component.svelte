	...
	<div class="container">
		<div class="footer-grid">
			...
			<div class="footer-section">
				<h4>Location</h4>
				<p>{address || "1234 Main Street, Brooklyn, NY 11201"}</p>
			</div>
			...
			<div class="footer-section">
				...
				{#if hours?.length}
					{#each hours as hour}
						<p class="hours">{hour.days}: {hour.time}</p>
					{/each}
				{/if}
			</div>
				...
				<div class="social-links">
					{#if social_links?.length}
						{#each social_links as social}
							<a href={social.url} aria-label={social.platform} target="_blank" rel="noopener">
		...
		<div class="footer-bottom">
			<p>{copyright || `© ${current_year} Sharp Cuts Barbershop. All rights reserved.`}</p>
		</div>
	</div>
</footer>
