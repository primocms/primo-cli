...
async function prompt_select(message: string, choices: { name: string; value: string; description?: string }[]): Promise<string> {
...
async function run_command(command: string, args: string[], cwd: string): Promise<{ success: boolean; output: string }> {
	return new Promise((resolve) => {
...
async function check_cli_installed(cli: string): Promise<boolean> {
	try {
...
async function build_static_site(site_dir: string, spinner: ReturnType<typeof ora>): Promise<string | null> {
	...
	const dev_promise = new Promise<void>((resolve) => {
...
async function publish_vercel(dist_dir: string, spinner: ReturnType<typeof ora>): Promise<boolean> {
...
async function publish_netlify(dist_dir: string, spinner: ReturnType<typeof ora>): Promise<boolean> {
...
async function publish_cloudflare(dist_dir: string, spinner: ReturnType<typeof ora>): Promise<boolean> {
	// Create _redirects for SPA routing
	await fs.writeFile(path.join(dist_dir, '_redirects'), '/*    /index.html   200\n')
...
async function publish_primo(site_dir: string, spinner: ReturnType<typeof ora>): Promise<boolean> {
...
export async function publish(options: PublishOptions) {
	...
	let saved_provider: string | null = null
