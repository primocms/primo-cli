<footer>
	<div class="container">
		<div class="footer-main">
			...
			{#if $site?.footer_link_groups?.length}
				<div class="footer-links">
					{#each $site.footer_link_groups as group}
						<div class="link-group">
							<h4>{group.title}</h4>
							{#if group.links?.length}
								<ul>
									{#each group.links as link_item}
										<li>
											<a href={link_item.link?.url}>{link_item.link?.label}</a>
										</li>
									{/each}
								</ul>
							{/if}
		...
		<div class="footer-bottom">
			<p class="copyright">{$site?.copyright || `© ${new Date().getFullYear()} All rights reserved.`}</p>
		</div>
	</div>
</footer>
