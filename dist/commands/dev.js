...
let initial_page_hashes = new Map();
let page_status = { new_pages: new Set(), updated_pages: new Set() };
function hash_page(page) {
...
async function regenerate(site_dir, dev_dir) {
    try {
        const blocks = await load_blocks(site_dir);
        const pages = await load_pages(site_dir);
        const site_content = await load_site_content(site_dir);
...
async function generate_toolbar(dev_dir, pages, page_type_configs, config) {
...
async function generate_vite_project(dev_dir, blocks, pages, site_content, page_type_layouts, page_type_configs, site_css, site_head, config) {
...
async function generate_blocks(dev_dir, blocks) {
    ...
    for (const [key, block] of Object.entries(blocks)) {
        ...
        if (written.has(component_name))
            continue;
        written.add(component_name);
        const svelte_code = assemble_block(block);
        await fs.writeFile(path.join(blocks_dir, `${component_name}.svelte`), svelte_code);
    }
}
function get_default_for_field_type(type) {
    switch (type) {
        case 'text':
        case 'textarea':
        case 'markdown':
...
function prosemirror_to_html(doc) {
...
function make_each_loops_safe(html) {
    ...
    return html.replace(/\{#each\s+([a-zA-Z_][\w.?]*)\s+as\b/g, (_match, expr) => {
...
function make_property_access_safe(html) {
    ...
    return html.replace(/\{([^}]*)\}/g, (_match, content) => {
        ...
        // - Strings: "some.text"
        // - Numbers: 1.5
        const transformed = content.replace(/\b([a-zA-Z_]\w*)\.([a-zA-Z_]\w*(?:\.[a-zA-Z_]\w*)*)\b/g, (propMatch, first, rest) => {
...
function assemble_block(block) {
    // Get unique fields with their types for defaults
    const seen_keys = new Set();
    const fields_with_types = [];
...
async function generate_pages(dev_dir, blocks, pages, site_content, page_type_layouts, page_type_configs) {
    ...
    for (const [page_path, page] of Object.entries(pages)) {
        ...
        await fs.writeFile(path.join(pages_dir, `${safe_name}.svelte`), page_code);
    }
}
function assemble_page(page, blocks, site_content, page_type_layouts, pages, page_type_configs) {
    ...
    if (layout?.header && layout.header.length > 0) {
        ...
        const header_sections = sorted_header
            .filter((item, idx) => {
    ...
    if (layout?.footer && layout.footer.length > 0) {
        ...
        const footer_sections = sorted_footer
            .filter((item, idx) => {
    ...
    const renders = sections.map((section, i) => {
        ...
        const site_field_names = new Map(); // field_name -> site_content_key
        const page_field_refs = new Map(); // field_name -> page field ID
        const page_list_fields = new Map(); // field_name -> { page_type: string }
        for (const field of (block.fields || [])) {
            const key = field.key || field.name;
            if (key) {
...
function build_layout_props(block, site_content) {
...
async function generate_app(dev_dir, pages, blocks, site_content, site_css, site_head, config) {
    ...
    page_entries.forEach(([page_path, page], i) => {
        ...
        if (page_path === 'index')
            route = '/';
        else if (page_path.endsWith('/index'))
            route = '/' + page_path.slice(0, -6);
...
function sanitize_component_name(name) {
...
async function load_blocks(site_dir) {
    const blocks = {};
...
function parse_svelte_component(code) {
    let html = code;
    let css = '';
    let js = '';
    // Extract style
...
async function load_pages(site_dir) {
    const pages = {};
    const pages_dir = path.join(site_dir, 'pages');
    async function scan_dir(dir, prefix = '') {
        try {
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        ...
                        if (ext === '.json') {
                            ...
                            pages[page_path] = yaml.load(data);
                        }
                    }
                    catch { }
                }
...
async function load_site_content(site_dir) {
...
async function load_page_type_configs(site_dir) {
    const configs = {};
...
async function load_page_type_layouts(site_dir) {
    const layouts = {};
    const page_types_dir = path.join(site_dir, 'page-types');
    try {
        const entries = await fs.readdir(page_types_dir, { withFileTypes: true });
...
async function read_file_or_empty(file_path) {
    try {
...
function strip_svelte_syntax(html) {
